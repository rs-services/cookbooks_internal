...
# Stops MySQL service
action :stop do
  service node[:db_percona][:service_name] do
    action :stop
  end
end
...
action :start do
  begin
    SystemTimer.timeout_after(node[:db_percona][:init_timeout].to_i) do
      begin
        service node[:db_percona][:service_name] do
          action :nothing
...
action :post_restore_cleanup do
  ...
  if snap_version > current_version
    ...
    if node[:db_percona][:enable_mysql_upgrade] == "true"
  ...
  # Compares size of node[:db_percona][:tunable][:innodb_log_file_size] to
  ...
  db_percona_set_mycnf "setup_mycnf" do
    ...
    compressed_protocol node[:db_percona][:compressed_protocol] ==
      "enabled" ? true : false
  end
...
action :install_client do
  ...
  case version
  when "5.1"
    node[:db_percona][:client_packages_install] = value_for_platform(
      ["centos", "redhat"] => {
  ...
  when "5.5"
    ...
    node[:db_percona][:client_packages_install] = value_for_platform(
      ["centos", "redhat"] => {
        "5.8" => ["Percona-Server-client-55", "Percona-Server-devel-55"],
        "default" => ["Percona-Server-client-55", "Percona-Server-devel-55"]
      },
      "ubuntu" => {
        "10.04" => [],
        "default" => ["percona-server-client-5.5", "libmysqlclient-dev"]
      },
      "default" => []
    )
  ...
  packages = node[:db_percona][:client_packages_uninstall]
  ...
  packages = node[:db_percona][:client_packages_install]
...
action :install_server do
  ...
  packages = node[:db_percona][:server_packages_uninstall]
  ...
  packages = node[:db_percona][:server_packages_install]
  packages.each do |p|
    package p
  end unless packages == ""
  ...
  ruby_block "clean innodb logfiles" do
    ...
    block do
      require 'fileutils'
      remove_files = ::Dir.glob(::File.join(node[:db_percona][:datadir], 'ib_logfile*')) + ::Dir.glob(::File.join(node[:db_percona][:datadir], 'ibdata*'))
  ...
  binlog = ::File.dirname(node[:db_percona][:log_bin])
  directory binlog do
    owner "mysql"
    group "mysql"
    recursive true
  ...
  directory node[:db_percona][:tmpdir] do
    owner "mysql"
    group "mysql"
    mode "0770"
    recursive true
...
bash "Patching /etc/init.d/mysql to add a quick sleep to prevent exit 1 on start" do
  ...
  code <<-EOH
    ...
    sleep 5 #to avoid exit1 on start" /etc/init.d/mysql
    else
  ...
  node[:db_percona][:ssl_enabled] =
    !node[:db_percona][:ssl][:ca_certificate].to_s.empty? &&
    !node[:db_percona][:ssl][:master_certificate].to_s.empty? &&
    !node[:db_percona][:ssl][:master_key].to_s.empty? &&
    !node[:db_percona][:ssl][:slave_certificate].to_s.empty? &&
    !node[:db_percona][:ssl][:slave_key].to_s.empty?

  node[:db_percona][:ssl_credentials] = {
    :ca_certificate => {
      :credential => node[:db_percona][:ssl][:ca_certificate],
      :path => "/etc/mysql/certs/ca_cert.pem"
    },
    :master_certificate => {
      :credential => node[:db_percona][:ssl][:master_certificate],
      :path => "/etc/mysql/certs/master_cert.pem"
    },
    :master_key => {
      :credential => node[:db_percona][:ssl][:master_key],
      :path => "/etc/mysql/certs/master_key.pem"
    },
    :slave_certificate => {
      :credential => node[:db_percona][:ssl][:slave_certificate],
      :path => "/etc/mysql/certs/slave_cert.pem"
    },
    :slave_key => {
      :credential => node[:db_percona][:ssl][:slave_key],
      :path => "/etc/mysql/certs/slave_key.pem"
    }
  }

  if node[:db_percona][:ssl_enabled]
    ...
    m_cmd << " #{node[:db_percona][:ssl_credentials][:ca_certificate][:path]}"
    m_cmd << " #{node[:db_percona][:ssl_credentials][:master_certificate][:path]}"

    s_cmd = "openssl verify -CAfile"
    s_cmd << " #{node[:db_percona][:ssl_credentials][:ca_certificate][:path]}"
    s_cmd << " #{node[:db_percona][:ssl_credentials][:slave_certificate][:path]}"
  ...
  log "  MySQL SSL enabled: #{node[:db_percona][:ssl_enabled]}"
  log "  MySQL SSL will only be enabled if all inputs contain credentials." do
    not_if { node[:db_percona][:ssl_enabled] }
  end
  ...
  mysql_file_ulimit = node[:db_percona][:file_ulimit]
  ...
  ruby_block "evaluate db type" do
    block do
      if node[:db][:init_status].to_sym == :initialized
        node[:db_percona][:collectd_master_slave_mode] = (node[:db][:this_is_master] == true ? "Master" : "Slave") + "Stats true"
      else
        node[:db_percona][:collectd_master_slave_mode] = ""
      end
    end
  end
...
action :grant_replication_slave do
  ...
  grant_query << " REQUIRE SSL" if node[:db_percona][:ssl_enabled]
...
action :enable_replication do
  ...
  node[:db_percona][:log_bin_enabled] = false
  ...
  directory ::File.dirname(node[:db_percona][:log_bin]) do
    not_if { current_restore_process == :no_restore }
