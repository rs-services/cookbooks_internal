    ...
    module MySQL
      module Helper
        ...
        def self.mycnf_uuid(node)
          node[:db_percona][:mycnf_uuid] = IPAddr.new(node[:cloud][:private_ips][0]).to_i
        end
        ...
        def self.mycnf_relay_log(node)
          node[:db_percona][:mycnf_relay_log] = Time.now.to_i.to_s + rand(9999).to_s.rjust(4, '0') if !node[:db_percona][:mycnf_relay_log]
          return node[:db_percona][:mycnf_relay_log]
        end
        ...
        def self.reconfigure_replication(
          ...
          if node[:db_percona][:ssl_enabled]
            cmd << ", MASTER_SSL=1"
            cmd << ", MASTER_SSL_CA="
            cmd << "'#{node[:db_percona][:ssl_credentials][:ca_certificate][:path]}'"
            cmd << ", MASTER_SSL_CERT="
            cmd << "'#{node[:db_percona][:ssl_credentials][:slave_certificate][:path]}'"
            cmd << ", MASTER_SSL_KEY="
            cmd << "'#{node[:db_percona][:ssl_credentials][:slave_key][:path]}'"
          end
