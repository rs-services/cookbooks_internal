...
action :stop do
  service node[:db_mongo][:service] do
    action :stop
  end
end
...
=begin
  ...
  packages = node[:db_mongo][:client_packages_uninstall]
  ...
  # Install Mongo client packages
  # Must install during the compile stage because mysql gem build depends on the libs
  if node[:platform] =~ /redhat|centos/
    # Install Mongo GPG Key (http://download.oracle.com/docs/cd/E17952_01/refman-5.5-en/checking-gpg-signature.html)
    gpgkey = ::File.join(::File.dirname(__FILE__), "..", "files", "centos", "mysql_pubkey.asc")
    `rpm --import #{gpgkey}`
  end

  packages = node[:db_mongo][:client_packages_install]
...
=begin
  # Start Mongo
  log "  Server installed.  Starting Mongo"
  db node[:db][:data_dir] do
    action :start
    persist false
  end
