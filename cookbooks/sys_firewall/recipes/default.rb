...
log "deciding whether to install logging, logging is #{node[:sys_firewall][:logging_enabled]}!" 
if node[:sys_firewall][:logging_enabled] == "enabled"
  log "enabling firewall logging"
  include_recipe "iptables"
    template "/etc/iptables.d/log" do 
      source "iptables_log.erb"
      variables(:fw_log_level => 4)
      not_if('test -e /etc/iptables.d/log')
    end
else 
  log "firewall logging disabled"
end
