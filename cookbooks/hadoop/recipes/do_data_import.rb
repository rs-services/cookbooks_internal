...
if skip
  ...
  bash "Unzip to #{node[:mapreduce][:destination]}" do
    flags "-ex"
    code <<-EOH
    unzip #{dumpfilepath} -d #{node[:mapreduce][:destination]}
    EOH
  ...
  bash "Copy to hadoop" do
    flags "-ex"
    code <<-EOH
      #{node[:hadoop][:install_dir]}/bin/hadoop fs  -copyFromLocal #{node[:mapreduce][:destination]}
    EOH
