...
old_hosts_array.each do |current_ip|
modify_cmd+=<<EOF
            <ResourceRecord>
...
EOF
        ...
        @logger.info(modify_cmd)
        @logger.info("Changing IP for #{hostname} from #{old_hosts_array.join(',')} to #{new_ip_array.join(',')}")
